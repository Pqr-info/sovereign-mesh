...
DB_PATH = os.path.join(HOME_DIR, "sovereign_mesh", "agent_pedigree.db")
...
def run_sys_cmd_stdin(args, stdin_data, require_root=True):
    cmd = (["sudo"] + args) if (require_root and os.geteuid() != 0) else args
    log(
        f"Executing system command (stdin): {' '.join(cmd)}",
        color=GOLD,
        prefix="SYS-CMD",
    )
...
class AgentSyncServicer(sync_pb2_grpc.AgentSyncServicer):
    ...
    def Ping(self, request, context):
        log(
            f"Ping received from client {BOLD}{request.client_id}{RESET} (TS: {request.timestamp})",
            color=BLUE,
        )
        return sync_pb2.PingResponse(
            server_id=self.node_id, timestamp=int(time.time() * 1000), status="ONLINE"
        )

    def HandshakeState(self, request, context):
        log(
            f"HandshakeState requested from {BOLD}{request.agent_id}{RESET}",
            color=MAGENTA,
        )
    ...
    def TeleportProcess(self, request, context):
        ...
        log(
            f"Process Teleportation Triggered: PID {BOLD}{pid}{RESET} -> {BOLD}{target}{RESET}",
            color=MAGENTA,
        )

        # Simulated silicon-level state extraction
        stack_trace = "main.py:12 -> silicon.py:88 -> radius_audit.py:42"

        return sync_pb2.TeleportProcessResponse(
            success=True,
            message=f"Process {pid} ({owner}) successfully materialized on {target}",
            stack_trace=stack_trace,
        )

    def AtomicSwap(self, request, context):
        target_pid = request.target_pid
        new_bin = request.new_binary_path

        log(
            f"🚀 ATOMIC SWAP: Hot-swapping PID {BOLD}{target_pid}{RESET} with upgraded binary {BOLD}{new_bin}{RESET}",
            color=RED,
            prefix="SECURITY",
        )

        # 1. State Preservation (Shared Memory Segment)
        state_id = request.state_segment_id or f"SEG-{target_pid}"
        log(
            f"  [+] State segment {BOLD}{state_id}{RESET} locked for handover.",
            color=GOLD,
        )

        # 2. Handoff Execution (Socket Transfer simulation)
        if request.transfer_sockets:
            log(
                f"  [+] Socket descriptors transferred to new PID context.", color=GREEN
            )

        time.sleep(0.5)  # Transition buffer
        new_pid = target_pid + 42

        log(
            f"✅ SWAP SUCCESS: New instance {BOLD}{new_pid}{RESET} is online and sentried.",
            color=GREEN,
        )

        return sync_pb2.AtomicSwapResponse(
            success=True,
            message=f"Hot-swap complete. Process {target_pid} logic replaced by {new_bin}.",
            new_pid=new_pid,
            handoff_status="OBULUSK_STABLE_FLIGHT",
        )

    def StreamInference(self, request, context):
        log(
            f"Remote inference requested for model {BOLD}{request.model}{RESET}",
            color=GREEN,
        )
        ...
        try:
            ...
            buffer = ""
            while (
                not context.is_active() or True
            ):  # Keep reading until finished or disconnected
                ...
                try:
                    ...
                    yield sync_pb2.InferenceChunk(
                        token=token, done=done, duration_ms=duration
                    )
                    if done:
            ...
            yield sync_pb2.InferenceChunk(
                token=f"[Inference Engine Error: {str(e)}]", done=True
            )
    ...
    def RemoteExecute(self, request, context):
        ...
        log(
            f"Execution request: {BOLD}{' '.join(full_command)}{RESET}",
            color=RED,
            prefix="SECURITY",
        )
        ...
        try:
            res = subprocess.run(
                full_command, capture_output=True, text=True, timeout=15
            )
    ...
    def CreateUser(self, request, context):
        ...
        if group:
            ...
            if not group_exists:
                log(
                    f"Group {group} does not exist. Creating group first...", color=BLUE
                )
                g_cmd = ["groupadd"]
                if group.isdigit():
        ...
        if password:
            ...
            if proc.returncode == 0:
                ...
                details.append(
                    f"Warning: User created but password configuration failed: {proc.stderr.strip()}"
                )
    ...
    def ManageGroup(self, request, context):
        ...
        log(
            f"ManageGroup request: User={username}, Group={group_name}, Action={action}, GID={gid}",
            color=GOLD,
        )
        details = []
        ...
        if action == sync_pb2.ManageGroupRequest.CREATE_GROUP:
            ...
            return sync_pb2.UserResponse(
                success=False, message="Unknown group action", details=[]
            )
    ...
    def SyncUsers(self, request, context):
        ...
        for u in request.users:
            ...
            if not user_exists:
                log(
                    f"Sync: User {username} does not exist. Creating user...",
                    color=BLUE,
                )
                ...
                if primary_group:
                    ...
                    if not group_exists:
                        ...
                        if res.returncode == 0:
                            details.append(
                                f"Sync: Created primary group {primary_group} with GID {gid}"
                            )
                        else:
                            details.append(
                                f"Sync error: Failed to create group {primary_group}: {res.stderr.strip()}"
                            )
                ...
                if res.returncode == 0:
                    details.append(
                        f"Sync: Created user {username} (UID: {uid}, GID: {gid})"
                    )
                    success_count += 1
                else:
                    details.append(
                        f"Sync error: Failed to create user {username}: {res.stderr.strip()}"
                    )
                    continue
            else:
                ...
                if uid > 0 and uid != existing_uid:
                    log(
                        f"  UID mismatch for {username}: local {existing_uid} vs requested {uid}. Aligning...",
                        color=GOLD,
                    )
                    mod_cmd += ["-u", str(uid)]
                    needs_update = True
                ...
                if needs_update:
                    ...
                    if res.returncode == 0:
                        details.append(
                            f"Sync: Updated properties for {username} (Aligned UID={uid}, GID={gid})"
                        )
                        if uid != existing_uid:
                            actual_home = home_dir or existing_home
                            chown_grp = primary_group or str(gid)
                            run_sys_cmd(
                                ["chown", "-R", f"{username}:{chown_grp}", actual_home]
                            )
                            details.append(
                                f"Sync: Recursively updated file ownership for {username} home directory: {actual_home}"
                            )
                    else:
                        details.append(
                            f"Sync error: Failed to update properties for {username}: {res.stderr.strip()}"
                        )
                ...
                for sg in groups:
                    if sg not in existing_groups and sg != primary_group:
                        ...
                        if res.returncode == 0:
                            details.append(
                                f"Sync: Added {username} to secondary group {sg}"
                            )
    ...
    def GetProcessDirectory(self, request, context):
        ...
        try:
            res = run_sys_cmd(
                ["ps", "-eo", "pid,ppid,user,stat,%cpu,%mem,comm,args", "--no-headers"],
                require_root=False,
            )
    ...
    def ManageProcess(self, request, context):
        ...
        log(
            f"ManageProcess request: PID {BOLD}{pid}{RESET}, Action {BOLD}{action}{RESET}",
            color=RED,
            prefix="SECURITY",
        )
        ...
        try:
            ...
            if action == "KILL":
                ...
                cmd = ["renice", str(priority), "-p", str(pid)]
            else:
                return sync_pb2.CommandResult(
                    exit_code=1, stderr=f"Unknown action: {action}"
                )

            res = run_sys_cmd(cmd, require_root=True)
            return sync_pb2.CommandResult(
                exit_code=res.returncode, stdout=res.stdout, stderr=res.stderr
            )
        except Exception as e:
            log(f"Process management failed: {e}", color=RED)
            return sync_pb2.CommandResult(exit_code=-1, stderr=str(e))

    def GetSystemMetrics(self, request, context):
        log("System Metrics requested - Diving to Silicon Layer", color=MAGENTA)
            ...
            with open("/proc/cpuinfo", "r") as f:
                ...
                for line in f:
                    if "cpu MHz" in line:
                        ...
                        cpu_cores.append(
                            sync_pb2.CPUMetrics(core_id=core_id, clock_mhz=mhz)
                        )
                        core_id += 1
                ...
                for zone in os.listdir(thermal_path):
                    if zone.startswith("thermal_zone"):
                        with open(f"{thermal_path}/{zone}/temp", "r") as f:
                            ...
                            if len(cpu_cores) > 0:
                                # Assign to first core for simplicity or distribute
                                cpu_cores[0].temperature_c = temp
        except Exception as e:
            log(f"Silicon metrics error: {e}", color=RED)

        memory = sync_pb2.MemoryMetrics()
        try:
            with open("/proc/meminfo", "r") as f:
                for line in f:
                    ...
                    if key == "MemTotal":
                        ...
                        memory.shared_kb = val
                    elif key == "Buffers":
                        memory.buffer_kb = val
                    elif key == "Cached":
                        memory.cached_kb = val
            memory.used_kb = memory.total_kb - memory.free_kb
        except Exception as e:
            log(f"Memory metrics error: {e}", color=RED)
        ...
        try:
            ...
            with open("/proc/uptime", "r") as f:
                uptime = f.read().split()[0] + "s"
            with open("/proc/loadavg", "r") as f:
                loads = f.read().split()
                load_avgs = [float(x) for x in loads[:3]]
        except:
            pass

        return sync_pb2.SystemMetricsResponse(
            cpu_cores=cpu_cores,
            memory=memory,
            kernel_version=kernel_version,
            uptime=uptime,
            load_avg_1=load_avgs[0],
            load_avg_5=load_avgs[1],
            load_avg_15=load_avgs[2],
        )

    def TeleportAgent(self, request, context):
        log(
            f"Virtual Travel: Agent teleporting {BOLD}{request.source_node_id}{RESET} -> {BOLD}{request.target_node_id}{RESET} over Memory Bus...",
            color=MAGENTA,
        )
        ...
        if not os.path.exists(path):
            log(
                f"Materialization failed: RAM Page table not allocated at {path}.",
                color=RED,
            )
        ...
        try:
            with open(path, "r+b") as f:
                ...
                log(
                    f"Agent mind successfully reconstructed from RAM page segment!",
                    color=GREEN,
                )
                log(f"  Mind Timestamp : {state_data.get('timestamp')}", color=CYAN)
                log(
                    f"  Model Engine   : {BOLD}{state_data.get('active_model')}{RESET}",
                    color=CYAN,
                )
                log(
                    f"  Source Host OS : {state_data.get('host_env', {}).get('os')}",
                    color=CYAN,
                )

                # 3. Resume / Execute command on target host
                log(
                    f"Executing migration resume command: {BOLD}{request.run_command}{RESET}",
                    color=GOLD,
                )
    ...
    def TracePedigree(self, request, context):
        ...
        log(
            f"Swarm Lineage: Tracing pedigree of '{BOLD}{agent_id}{RESET}' back to Agent 0...",
            color=MAGENTA,
        )
        ...
        try:
            ...
            cursor.execute(
                "SELECT agent_id, name, layer_level, specialty, subspecialty FROM agents ORDER BY layer_level ASC"
            )
            ...
            for lvl in sorted(layers.keys()):
                ...
                for ag in layers[lvl]:
                    coop_agents = [
                        x[0] for x in all_agents if x[3] == ag[3] and x[0] != ag[0]
                    ]
                    coop_str = (
                        f" [Cooperates with: {', '.join(coop_agents)}]"
                        if coop_agents
                        else ""
                    )
                    tree_lines.append(
                        f"  ├── {BOLD}{ag[0]}{RESET} ({ag[1]}) - Specialty: {BOLD}{ag[3]}{RESET} -> Subspecialty: {ag[4]}{GOLD}{coop_str}{RESET}"
                    )

            coop_map = "\n".join(tree_lines)

            log(
                f"Pedigree trace successfully completed with {len(path)} lineage nodes.",
                color=GREEN,
            )
            return sync_pb2.PedigreeResponse(
                pedigree_path=path, collective_specialty_cooperation_map=coop_map
            )
        except Exception as e:
    ...
    def ProposeSwarmMutation(self, request, context):
        ...
        log(
            f"Swarm Mutation: Proposing key modification: '{BOLD}{target_key}{RESET}' -> '{BOLD}{proposed_value}{RESET}' proposed by '{proposer}'...",
            color=MAGENTA,
        )
        ...
        for agent_id, name, specialty in voters:
            agree = True
            rationale = (
                "Swarm alignment verified. Mutation enhances operational telemetry."
            )

            # Security Overlord is highly protective of core settings
            if agent_id == "AGENT-SEC" and any(
                x in target_key.lower()
                for x in ["security", "port", "root", "password", "key"]
            ):
                agree = False
            ...
            votes.append(
                sync_pb2.SwarmVote(
                    agent_id=agent_id, vote_agree=agree, rationale=rationale
                )
            )
        ...
        if agree_count < 5:
            ...
            minority_reasons = "\n".join(
                [f" - {v.agent_id}: '{v.rationale}'" for v in votes if not v.vote_agree]
            )
...
* [DECISION]: {"MUTATION APPROVED under monitored namespace." if consensus_reached else "MUTATION REJECTED due to consensus deficit (< 4/5)."}"""
            ...
            if consensus_reached:
                ...
                """,
                    (
                        ...
                        target_key,
                        proposed_value,
                        f"State parameter '{target_key}' modified to '{proposed_value}'.",
                        proposer,
                        ts_now,
                    ),
                )
                ...
                cursor.execute(
                    "INSERT OR REPLACE INTO master_knowledge VALUES (?, ?, ?)",
                    (target_key, proposed_value, proposer),
                )

                # Retrieve last block hash
                cursor.execute(
                    "SELECT block_hash FROM ledger ORDER BY block_index DESC LIMIT 1"
                )
                ...
                cursor.execute(
                    "INSERT INTO ledger (block_index, previous_hash, timestamp, agent_id, mutation_payload, consensus_votes, minority_report, block_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        new_idx,
                        prev_hash,
                        ts_now,
                ...
                status = "COMMITTED"
                log(
                    f"Mutation committed successfully at Ledger Block #{new_idx}! Block Hash: {block_hash[:16]}...",
                    color=GREEN,
                )
            else:
                # Reject ticket
                ...
                log(
                    f"Mutation rejected: Consensus ratio of {consensus_ratio} fails 4/5 baseline requirement.",
                    color=RED,
                )
    ...
    def QuerySwarmLedger(self, request, context):
        log(
            "Ledger Audit: Retrieving all blocks from multi-dimensional immutable ledger...",
            color=MAGENTA,
        )
        ...
        try:
            ...
            return sync_pb2.LedgerQueryResponse(
                blocks=blocks, chain_validation_status=chain_valid
            )
        except Exception as e:
            log(f"Ledger Query failed: {e}", color=RED)
            return sync_pb2.LedgerQueryResponse(
                blocks=[], chain_validation_status=f"FAILED: {e}"
            )
    ...
    def TimeTravelOverride(self, request, context):
        ...
        log(
            f"⏰ JETWEB TIME MACHINE: Activating timeline refactoring. Destination Block #{target_idx}...",
            color=CYAN,
        )
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM ledger WHERE block_index = ?", (target_idx,)
            )
            ...
            ts_now = datetime.now().isoformat()
            subject = (
                f"Timeline Override: Block #{target_idx} rewrite to {new_key}={new_val}"
            )
            task_desc = (
                f"Time Machine Override: set {new_key}={new_val} at block #{target_idx}"
            )
            ...
            """,
                (
                    ticket_id,
                    f"Block #{target_idx} legacy state",
                    f"Block #{target_idx} overridden to {new_key}={new_val}",
                    f"Re-mined blockchain cascade starting at Block #{target_idx}.",
                    ts_now,
                ),
            )
            ...
            new_payload = json.dumps(
                {
                    ...
                    "key": new_key,
                    "value": new_val,
                    "reason": f"[JETWEB OVERRIDE]: {reason}",
                }
            )
            ...
            new_votes = json.dumps(
                [
                    {
                        "agent_id": "JETWEB-ADMIN",
                        "vote_agree": True,
                        "rationale": f"Time Machine administrative override: {reason}",
                    }
                ]
            )
            ...
            for idx, block in enumerate(blocks):
                ...
                if block_num < target_idx:
                    ...
                    refactor_logs.append(
                        f"Mined Overridden Block #{block_num}: New Hash: {block[7][:16]}..."
                    )
                    prev_hash = block[7]
                    ...
                    refactor_logs.append(
                        f"Re-signed Block #{block_num}: Previous Hash: {block[1][:12]}... -> New Hash: {block[7][:12]}..."
                    )
                    prev_hash = block[7]
            ...
            cursor.execute(
                "INSERT INTO master_knowledge VALUES ('swarm_status', 'ONLINE', 'AGENT-0')"
            )
            ...
            for block in blocks:
                ...
                if "key" in payload and "value" in payload:
                    ...
                    refactor_logs.append(
                        f"  Applied committed key '{payload['key']}' = '{payload['value']}'"
                    )
            ...
            refactor_logs.append(
                "⏰ Timeline collapse completed successfully. State tree harmonized."
            )
            log(
                "Jetweb Time Machine: Timeline collapsed and merged successfully.",
                color=GREEN,
            )
            return sync_pb2.TimeTravelResponse(
                success=True,
    ...
    def ForensicAudit(self, request, context):
        target_idx = request.target_block_index
        log(
            f"🕵️ FORENSIC ACCOUNTING: Retroactively auditing system timeline. Filter Index: {target_idx or 'ALL'}...",
            color=MAGENTA,
        )
        ...
        try:
            ...
            for k in dump_rows:
                knowledge_list.append(
                    f"  🔑 '{k[0]}' = '{k[1]}' (Last Mutation Agent: {k[2]})"
                )
            ...
            return sync_pb2.ForensicResponse(
                timeline_nodes=timeline_nodes, master_knowledge_dump=master_dump
            )
        except Exception as e:
            log(f"Forensic audit failed: {e}", color=RED)
            return sync_pb2.ForensicResponse(
                timeline_nodes=[], master_knowledge_dump=f"ERROR: {e}"
            )

    def GetStarchart(self, request, context):
        log(
            "Starchart Request: Synthesizing Jetweb Time Machine & RADIUS data...",
            color=CYAN,
        )
        ...
        try:
            ...
            # 1. Fetch Timeline (Jetweb Time Machine blocks)
            cursor.execute(
                "SELECT block_index, previous_hash, timestamp, agent_id, mutation_payload, consensus_votes, minority_report, block_hash FROM ledger ORDER BY block_index DESC LIMIT 50"
            )
            ...
            # 2. Fetch Accounting Data (RADIUS logs)
            cursor.execute(
                "SELECT username, session_id, status_type, input_octets, output_octets, timestamp FROM accounting ORDER BY id DESC LIMIT 50"
            )
            ...
            # 3. Generate Constellation Map (SVG)
            constellation = f"""
            <svg viewBox="0 0 800 400" xmlns="http://www.w3.org/2000/svg">
                <rect width="800" height="400" fill="#030712" />
                <style>
                    .node {{ fill: #06b6d4; stroke: #fff; stroke-width: 2; }}
                    .link {{ stroke: #94a3b8; stroke-opacity: 0.4; }}
                    .text {{ fill: #f3f4f6; font-size: 12px; font-family: 'JetBrains Mono', monospace; }}
                    @keyframes pulse {{ 0% {{ opacity: 0.4; }} 100% {{ opacity: 1; }} }}
                    .pulse {{ animation: pulse 2s infinite alternate; }}
                </style>
                <line x1="100" y1="200" x2="400" y2="100" class="link pulse" />
                <line x1="100" y1="200" x2="400" y2="300" class="link pulse" />
                <line x1="400" y1="100" x2="700" y2="200" class="link pulse" />
                <line x1="400" y1="300" x2="700" y2="200" class="link pulse" />
                
                <circle cx="100" cy="200" r="12" class="node" />
                <text x="70" y="235" class="text">39.MH (SENTRY)</text>
                
                <circle cx="400" cy="100" r="12" class="node" />
                <text x="370" y="85" class="text">38.MH (FORGE)</text>
                
                <circle cx="400" cy="300" r="12" class="node" />
                <text x="370" y="335" class="text">201.MH (EDGE)</text>
                
                <circle cx="700" cy="200" r="12" class="node" />
                <text x="670" y="235" class="text">GCP (OPS)</text>
            </svg>
            """

            return sync_pb2.StarchartResponse(
                timeline=timeline,
            ...
            log(f"Starchart synthesis failed: {e}", color=RED)
            return sync_pb2.StarchartResponse(
                timeline=[], accounting_data=[], constellation_map=f"ERROR: {e}"
            )
    ...
    def ProvisionNode(self, request, context):
        ...
        log(
            f"☁️ INFRA ORCHESTRATION: Provisioning {BOLD}{node_class}{RESET} on {BOLD}{provider}{RESET} ({region})...",
            color=MAGENTA,
        )

        # Simulated multi-cloud orchestration
        if provider == "GCP":
            instance_id = f"gcp-{node_class.lower()}-01"
            ip = "34.123.50.201"
            ...
            return sync_pb2.ProvisionNodeResponse(
                success=False, message=f"Unsupported provider: {provider}"
            )

        log(
            f"✅ PROVISION SUCCESS: Instance {BOLD}{instance_id}{RESET} active at {BOLD}{ip}{RESET}",
            color=GREEN,
        )

        # Log to Starchart via accounting table
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            cursor.execute(
                """
                INSERT INTO accounting (username, session_id, status_type, input_octets, output_octets, timestamp)
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (
        ...
        return sync_pb2.ProvisionNodeResponse(
            ...
            public_ip=ip,
            message=f"Node materialized on {provider} cloud.",
        )

    def ExecuteStrike(self, request, context):
        log(
            f"🧨 STRIKE PROTOCOL: Ticket {BOLD}{request.ticket_id}{RESET} | Payload: {request.logic_payload}",
            color=RED,
            prefix="SECURITY",
        )
        return sync_pb2.StrikeResponse(
            exit_code=0,
            log_output=f"Strike executed successfully for {request.ticket_id}",
            proof_hash="reconciled-proof-hash",
        )

    def StreamVitality(self, request, context):
        log(
            f"📈 VITALITY STREAM: Starting for {BOLD}{request.node_id}{RESET}",
            color=BLUE,
        )
        while context.is_active():
            yield sync_pb2.TelemetryData(vitality_score=7.2, slope=5.16)
            time.sleep(2)

    def SyncBlackhole(self, request_iterator, context):
        log("🌑 BLACKHOLE SYNC: Bi-directional stream active.", color=BOLD)
        for update in request_iterator:
            yield update

    def ExecuteShell(self, request, context):
        return self.RemoteExecute(request, context)

    def StreamLogs(self, request, context):
        log(f"📜 LOG STREAM: Initiating for {request.node_id}", color=GOLD)
        while context.is_active():
            yield sync_pb2.LogEntry(message="[SYSTEM-LOG] Reconciled stream active.")
            time.sleep(5)
    ...
    def UpdateDNS(self, request, context):
        ...
        log(
            f"🌐 DNS ORCHESTRATION: Updating {provider} {request.record_type} record: {BOLD}{name}{RESET} -> {BOLD}{content}{RESET}",
            color=CYAN,
        )

        # Simulated API call to Cloudflare/GoDaddy
        log(
            f"✅ DNS SUCCESS: Record {BOLD}{name}{RESET} is live and propagated.",
            color=GREEN,
        )

        return sync_pb2.UserResponse(
            success=True, message=f"DNS record {name} updated on {provider}."
        )
    ...
    def ManageTunnel(self, request, context):
        ...
        log(
            f"🛡️ TUNNEL ORCHESTRATION: {BOLD}{action}{RESET} tunnel {BOLD}{name}{RESET}...",
            color=CYAN,
        )

        # Simulated Cloudflare Tunnel management
        log(
            f"✅ TUNNEL SUCCESS: Tunnel {BOLD}{name}{RESET} {action} complete.",
            color=GREEN,
        )

        return sync_pb2.UserResponse(
            success=True, message=f"Cloudflare Tunnel {name} {action} successful."
        )
    ...
    def CreateTicket(self, request, context):
        ...
        return sync_pb2.UserResponse(
            success=True, message=f"Ticket {ticket_id} ledgered."
        )

    def RecordAccounting(self, request, context):
        log(
            f"RADIUS Accounting Persistence: {request.username} ({request.status_type})",
            color=BLUE,
        )
        ...
        try:
            ...
            conn.close()
            return sync_pb2.SyncAck(
                success=True, message="Accounting record committed to Starchart DB"
            )
        except Exception as e:
            log(f"Accounting persistence failed: {e}", color=RED)
            return sync_pb2.SyncAck(success=False, message=str(e))


# DB_PATH is now defined globally relative to HOME
    ...
    try:
        ...
        if cursor.fetchone()[0] == 0:
            seed_data = [
                (
                    "AGENT-0",
                    "Agent 0 (Sovereign Swarm Root)",
                    None,
                    1,
                    "Core Swarm Mind",
                    "Progenitor Swarm Mind",
                ),
                (
                    "AGENT-SEC",
                    "Security Overlord",
                ...
                (
                    ...
                    3,
                    "Remote Action",
                    "High-Speed Memory Teleportation",
                ),
                (
                    "AGENT-USERADD",
                    "User Builder",
                ...
                (
                    "AGENT-SS-REG",
                    "Socket Regex Matcher",
                    "AGENT-SS",
                    5,
                    "System Diagnostics",
                    "ss userpid Regex Parser",
                ),
                (
                    "AGENT-MMAP",
                    "mmap Direct Buffer",
                ...
                (
                    ...
                    6,
                    "Security & Compliance",
                    "subprocess.run Spawner",
                ),
                (
                    "LAPTOP-TRAINING-AGENT",
                    "Laptop Training Node Agent",
                    "AGENT-SYS-EXEC",
                    7,
                    "Security & Compliance",
                    "gRPC / SSH Dial plane",
            ...
            cursor.executemany(
                "INSERT INTO agents VALUES (?, ?, ?, ?, ?, ?)", seed_data
            )
            conn.commit()
            log(
                "[PEDIGREE-DB] Database successfully seeded with 7-layer pedigree graph.",
                color=GREEN,
            )
        ...
        if cursor.fetchone()[0] == 0:
            genesis_payload = json.dumps(
                {"action": "GENESIS_INITIALIZATION", "state": "ACTIVE"}
            )
                ...
                [
                    {
                        "agent_id": "AGENT-0",
                        "vote_agree": True,
                        "rationale": "Swarm Root Progenitor Genesis block constructed.",
                    }
                ]
            )
            import hashlib
            ...
            cursor.execute(
                "INSERT INTO ledger (block_index, previous_hash, timestamp, agent_id, mutation_payload, consensus_votes, minority_report, block_hash) VALUES (1, '0', ?, 'AGENT-0', ?, ?, 'None', ?)",
                (ts, genesis_payload, genesis_votes, genesis_hash),
            )

            cursor.execute(
                "INSERT OR REPLACE INTO master_knowledge VALUES ('swarm_status', 'ONLINE', 'AGENT-0')"
            )
            conn.commit()
            log(
                "[LEDGER-GENESIS] Immutable ledger initialized and Genesis Block mined successfully.",
                color=GREEN,
            )
...
def serve():
    ...
    log(
        "🐍 OUROBOROS SENTINEL (Python) active. Monitoring mesh vitality.", color=PURPLE
    )
    print(f"""
{GOLD} ▄████▄   ██▀███   ██▓███   ▄▄▄▄    ██▀███   ▒█████   ██▓     ▒█████   ▒█████  
▒██▀ ▀█  ▓██ ▒ ██▒▓██░  ██▒▓█████▄ ▓██ ▒ ██▒▒██▒  ██▒▓██▒    ▒██▒  ██▒▒██▒  ██▒
...
▒▓▓▄ ▄██▒▒██▀▀█▄  ▒██▄█▓▒ ▒▒██░█▀  ▒██▀▀█▄  ▒██   ██░▒██░    ▒██   ██░▒██   ██░
 ...
 ░                                ░                                             
    ...
    server.start()
    log(
        f"gRPC server is running and listening. Press Ctrl+C to terminate.", color=GREEN
    )
...
class NeuralTrainingServicer(sync_pb2_grpc.NeuralTrainingServicer):
    def __init__(self):
        log("🧠 Neural Training Servicer initialized", color=MAGENTA)
        self.sessions = {}

    def InitiateTraining(self, request, context):
        session_id = f"session-{request.model_name}-{int(time.time())}"
        log(
            f"🧠 NEURAL INITIATION: Starting {request.model_name} on {request.cluster_id}",
            color=MAGENTA,
        )
        ...
        self.sessions[session_id] = {
            ...
            "loss": 0.0,
            "phase_drift": 0.0,
            "gradient_vitality": 5.0,
        }

        return sync_pb2.TrainingSession(
            session_id=session_id, cluster_id=request.cluster_id, status="RUNNING"
        )
    ...
    def GetTrainingStatus(self, request, context):
        ...
        return sync_pb2.TrainingStatus(
            session_id=session["session_id"],
            current_step=session["current_step"],
            loss=session["loss"],
            phase_drift=session["phase_drift"],
            gradient_vitality=session["gradient_vitality"],
            state=session["status"],
        )


class SovereignCityServicer(sync_pb2_grpc.SovereignCityServicer):
    def __init__(self):
        log("🏙️ Sovereign City Servicer initialized", color=GREEN)
        self.citizens = {}

    def RegisterCitizen(self, request, context):
        citizen_id = f"CITIZEN-{int(time.time())}"
        log(f"🛂 CITY REGISTRATION: {request.username}", color=GREEN)
        self.citizens[citizen_id] = {
            "username": request.username,
            "balance": request.initial_burn_amount,
        }
        return sync_pb2.CitizenPassport(
            citizen_id=citizen_id, access_token=f"jwt-{citizen_id}", status="ACTIVE"
        )

    def RequestService(self, request, context):
        log(
            f"📦 CITY SERVICE: {request.citizen_id} requesting {request.service_type}",
            color=CYAN,
        )
        return sync_pb2.ServiceAllocation(service_id="SRV-123", endpoint="city.mesh")

    def GetCitizenStatus(self, request, context):
        return sync_pb2.CitizenStatusResponse(
            citizen_id=request.citizen_id, surfgo_balance=100.0, status="ACTIVE"
        )
